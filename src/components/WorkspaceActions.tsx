    ...
    <div className="workspace-actions" aria-label="当前 SSH 会话操作">
      {session ? <>
        ...
        <div className="workspace-more">
          <button className={`workspace-action ${moreOpen ? "active" : ""}`} onClick={() => setMoreOpen((value) => !value)} aria-expanded={moreOpen} aria-label="更多当前会话操作">
            <MoreHorizontal size={15} /> 更多
          </button>
          {moreOpen && <div className="workspace-more-menu">
            <button onClick={() => { setMoreOpen(false); onOpenMonitor(); }}><Activity size={14} /> 监控面板</button>
            <button onClick={() => { setMoreOpen(false); onOpenTasks("forwards"); }}><GitPullRequestArrow size={14} /> 端口转发</button>
            <button onClick={() => { setMoreOpen(false); onOpenTasks("transfers"); }}><PanelRight size={14} /> 传输任务</button>
            <button className="danger" onClick={() => { setMoreOpen(false); onDisconnect(); }}><LogOut size={14} /> 断开连接</button>
          </div>}
        </div>
        </div>
      ...
      <div className="workspace-global-actions">
        ...
        <button className="workspace-icon-action" title="命令面板 (⌘K)" aria-label="打开命令面板" onClick={onOpenCommandPalette}><Search size={15} /></button>
