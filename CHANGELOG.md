...
### 修复
- **启动钥匙串连弹**：凭据加密缓存支持跨进程落盘（本机绑定 AES），工作区恢复前批量预热凭据，避免每次打开连续弹多次 macOS 授权框。
- **终端排版错乱**：非活动 Tab 改用 `visibility` 隐藏（保留布局尺寸）；Tab 激活时强制 fit + 同步远端 PTY；日志高亮收紧匹配规则并改用流式 UTF-8 解码，减少对 vim/htop/中文输出的误伤。

### 优化
- **终端体验**：默认滚动缓冲 10000 行、选中即复制、右键选词、Option-as-Meta；设置面板可调滚动缓冲 / 复制行为 / 日志高亮开关。
